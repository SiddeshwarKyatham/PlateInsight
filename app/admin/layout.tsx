...
const navItems = [
  ...
  { icon: TrendingDown, label: "Waste Trends", href: "/admin/waste-trend" },
  { icon: PieChart, label: "Dish Analytics", href: "/admin/dish-analytics" },
  { icon: FileText, label: "Reports", href: "/admin/reports" },
  { icon: Users, label: "Staff Directory", href: "/admin/staff" },
];
       ...
       if (user) {
          ...
          if (profile?.ecosystem_id) {
            const { data: eco } = await supabase
              .from('ecosystems')
              .select('ecosystem_code')
              .eq('id', profile.ecosystem_id)
              .single();
            if (eco?.ecosystem_code) {
              setEcoCode(eco.ecosystem_code);
            }
          }
       }
    };
    ...
    <div className="min-h-screen bg-gray-50 flex">
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <header className="bg-white/80 backdrop-blur-md border-b border-gray-100 h-20 flex items-center justify-between px-4 lg:px-8 sticky top-0 z-30">
          ...
          <div className="flex items-center gap-4">
            {isMounted && ecoCode && typeof ecoCode === 'string' && (
              <div 
                onClick={async () => {
                  await navigator.clipboard.writeText(ecoCode);
                  setCopied(true);
                  setTimeout(() => setCopied(false), 1200);
                }}
                className="hidden lg:flex items-center gap-2 px-3 py-1.5 bg-primary/10 border border-primary/20 rounded-lg cursor-pointer hover:bg-primary/20 transition-all text-primary shadow-sm max-w-[420px]"
                title="Click to copy Ecosystem Code"
              >
                <div className="text-xs font-bold font-mono break-all leading-tight">
                  Code: <span className="opacity-90">{ecoCode}</span>
                </div>
            ...
            {isMounted && (
              <div className="flex items-center gap-4 cursor-pointer hover:opacity-80 transition-opacity">
                <div className="hidden md:block text-right">
                  ...
                  <div className="text-xs font-semibold text-primary uppercase tracking-wider">Admin</div>
                </div>
